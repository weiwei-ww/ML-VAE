    ...
    def compute_forward(self, batch, stage):
        ...
        if self.target in [Target.PHN_RECOG, Target.VAE, Target.TEST]:
            ...
            phn_recog_out_dict = self.modules['phoneme_recognizer'](
                phn_recog_in_fc_out, feat_lens, plvl_cnnl_phn_seqs, plvl_cnnl_phn_seq_lens, fa_boundary_seqs
            )
        ...
        if self.target in [Target.B_DETECTOR, Target.VAE, Target.TEST]:
            ...
            fa_boundary_seqs = batch['fa_boundary_seq'][0]
            b_detector_out = self.modules['boundary_detector'](b_detector_in_fc_out, feat_lens, fa_boundary_seqs)
        ...
        if self.target in [Target.VAE, Target.TEST]:
            ...
            # concatenation
            rnn_in = torch.cat([w2v_feat_fc_out, phn_recog_out_fc_out], dim=-1)
