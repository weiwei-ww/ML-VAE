...
class SBModel(MDModel):
    ...
    def compute_forward(self, batch, stage):
        ...
        predictions['phn_recog_out'] = phn_recog_out_dict['out'].detach().cpu()
        predictions['losses'].update(phn_recog_out_dict['losses'])

        # FC for phoneme recognizer output
        phn_recog_fc_out = self.modules['phn_recog_fc'](phn_recog_out_dict['out'])

        # boundary detector
        fa_boundary_seqs = batch['fa_boundary_seq'][0]
        b_detector_out = self.modules['boundary_detector'](feats, feat_lens, fa_boundary_seqs)
        predictions['boundary_v'] = b_detector_out['boundary_v'].detach().cpu()
        ...
        predictions['pi_logits'] = pi_logits.detach().cpu()
        ...
        predictions['sampled_pi'] = sampled_pi.detach().cpu()
