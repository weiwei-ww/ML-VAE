...
class SBModel(MD_VAE):
    def on_stage_start(self, stage, epoch=None):
        ...
        if stage == sb.Stage.TRAIN or stage == sb.Stage.VALID:
            ...
            train_targets = [Target.B_DETECTOR, Target.VAE]
            self.target = train_targets[(epoch - 1) % 2]
        ...
        # load pre-saved phoneme recognizer outputs
        phn_recog_outs_path = Path(self.hparams.output_dir).parent / self.hparams.phn_recog_outs_path
        self.saved_phn_recog_outs = torch.load(phn_recog_outs_path)
    ...
    def compute_forward(self, batch, stage):
        ...
        if self.target in [Target.VAE, Target.TEST]:
            ...
            # get pre-saved phoneme recognizer outputs
            utt_ids = batch['id']
            phn_recog_outs = [self.saved_phn_recog_outs[utt_id] for utt_id in utt_ids]
            phn_recog_outs = pad_sequence(phn_recog_outs, batch_first=True).to(self.device)
            assert phn_recog_outs.shape[:2] == feats.shape[:2]
            predictions['phn_recog_out'] = phn_recog_outs

            # FC for phoneme recognizer output
            phn_recog_out_fc_out = self.modules['phn_recog_out_fc'](phn_recog_outs.detach())
